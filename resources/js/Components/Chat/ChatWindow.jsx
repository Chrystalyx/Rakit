...
export default function ChatWindow({ activeChat, onBack, className = "" }) {
    ...
    useEffect(() => {
        if (activeChat) {
            setLoading(true);
            // Panggil API Laravel: Route::get('/chat/messages/{user}')
            axios
    ...
    // Scroll setiap kali ada pesan baru ditambahkan
    useEffect(() => {
        scrollToBottom();
    }, [messages]);
    ...
    const handleSendMessage = (e) => {
        ...
        // Optimistic UI: Tampilkan dulu pesan di layar seolah sudah terkirim (biar cepat)
        ...
        // Simpan pesan sementara ke state
        const currentMsg = newMessage; // Simpan teks untuk dikirim
        setNewMessage(""); // Kosongkan input segera

        // Kirim ke API Laravel: Route::post('/chat/messages/{user}')
        axios
            .post(`/chat/messages/${activeChat.id}`, { message: currentMsg })
            .then((response) => {
                // Sukses: Tambahkan pesan asli dari server ke state
                setMessages((prev) => [...prev, response.data]);
            })
            .catch((error) => {
                console.error("Gagal mengirim pesan:", error);
                // Di sini bisa tambahkan logika retry atau notifikasi error
            });
    };
    ...
    return (
        <div
            className={`flex flex-col h-full bg-[#f0eadd]/30 relative ${className}`}
        >
            {/* Chat Header */}
            ...
            <div
                ...
                style={{
                    backgroundImage:
                        "radial-gradient(#d6d1cb 1px, transparent 1px)",
                    ...
                    <>
                        {messages.length === 0 && (
                            <div className="text-center text-gray-400 text-sm mt-10">
                                Belum ada pesan. Sapa {activeChat.name}{" "}
                                sekarang!
                            </div>
                        )}
                        {messages.map((msg) => (
