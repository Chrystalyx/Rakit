...
import {
    ...
    ThumbsUp,
    ShieldCheck,
    Star, // <--- PERBAIKAN: Star sudah ditambahkan di sini
} from "lucide-react";

import Visualizer from "../Customize/Visualizer";

// --- COMPONENT: MODAL FORM PORTOFOLIO (CRUD) ---
const PortfolioFormModal = ({ isOpen, onClose, portfolio, onSave }) => {
    // State form awal
    const [formData, setFormData] = useState({
        title: portfolio?.title || "",
        category: portfolio?.category || "Kitchen Set",
        date: portfolio?.date || "Baru Selesai",
        description: portfolio?.description || "",
        // Default Config untuk Visualizer
        ...
        specs: portfolio?.specs || {
            material: "Multiplek 18mm",
            finish: "HPL Taco",
        },
        realPhotos: portfolio?.realPhotos || [],
    });
    ...
    const handleSubmit = (e) => {
        ...
        const finalData = {
            ...formData,
            realPhotos:
                formData.realPhotos.length > 0
                    ? formData.realPhotos
                    : [
                          {
                              view: "Tampak Depan",
                              url: "https://images.unsplash.com/photo-1595428774223-ef52624120d2?q=80&w=400",
                          },
                          {
                              view: "Detail",
                              url: "https://images.unsplash.com/photo-1558997519-83ea9252edf8?q=80&w=400",
                          },
                      ],
        };
        onSave(finalData);
            ...
            >
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white z-10">
                    <h3 className="text-lg font-bold text-gray-900">
                        {portfolio ? "Edit Proyek" : "Tambah Proyek Baru"}
                    </h3>
                    <button
                        onClick={onClose}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-2 rounded-lg border-gray-300 focus:border-rakit-500 focus:ring-rakit-500 text-sm"
                                        required
                                    />
                                </div>
                                <div>
                ...
                <div className="p-5 border-t border-gray-100 bg-white flex justify-end gap-3 z-10">
                    ...
                    <button
                        type="submit"
                        form="portfolio-form"
                        className="px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-rakit-800 hover:bg-rakit-900 shadow-lg"
                    >
                        Simpan
                    </button>
                </div>
    ...
    <div className="bg-white rounded-[2rem] border border-gray-200 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 relative group">
        <div className="absolute top-4 right-4 z-20 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            <button
                ...
                className="p-2.5 bg-white text-gray-700 hover:text-blue-600 rounded-full shadow-lg border border-gray-100 hover:scale-110 transition"
            >
                <Edit2 size={18} />
            </button>
            <button
                onClick={() => onDelete(project.id)}
                className="p-2.5 bg-white text-gray-700 hover:text-red-600 rounded-full shadow-lg border border-gray-100 hover:scale-110 transition"
            >
                <Trash2 size={18} />
            </button>
        </div>
            ...
            <div className="w-full xl:w-7/12 p-6 bg-white">
                ...
                <div className="grid grid-cols-4 gap-3">
                    {project.realPhotos.map((photo, idx) => (
                        <div
                            key={idx}
                            className="aspect-square rounded-lg overflow-hidden border border-gray-100 relative group/img"
                        >
                            <img
                                src={photo.url}
                                alt={photo.view}
...
export default function Portfolio({ crafter }) {
    const [profileData, setProfileData] = useState(crafter);
    const [portfolios, setPortfolios] = useState(crafter.portfolios);
    ...
    const handleSavePortfolio = (newData) => {
        if (editingItem) {
            setPortfolios((prev) =>
                prev.map((p) =>
                    p.id === editingItem.id ? { ...p, ...newData } : p
                )
            );
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex justify-end gap-2">
                                            ...
                                            <button
                                                ...
                                                className="px-4 py-2 text-xs font-bold text-white bg-rakit-800 hover:bg-rakit-900 rounded-lg shadow-sm"
                                            >
                                                Simpan
                                            </button>
                                        </div>
                        ...
                        <div className="lg:col-span-1 space-y-6">
                            ...
                            <div className="bg-white p-4 rounded-2xl border border-gray-200 shadow-sm">
                                <Link
                                    href="/crafter/view/1"
                                    className="block w-full py-3 text-center bg-gray-50 border border-gray-200 text-gray-600 font-bold text-sm rounded-xl hover:bg-gray-100 transition"
                                >
