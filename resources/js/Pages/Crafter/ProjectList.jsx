                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div>
                            <div className="flex items-center gap-3">
                                ...
                                <span className="px-3 py-1 bg-rakit-100 text-rakit-800 text-xs font-bold rounded-full border border-rakit-200 flex items-center gap-1 uppercase">
                                    <CheckCircle2 size={12} className="text-rakit-600" />
                                    {crafter?.level || "Mitra"}
                                </span>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-rakit-200 shadow-sm group hover:border-rakit-300 transition">
                            ...
                            <h3 className="text-3xl font-bold text-rakit-800">{crafter?.active_projects || 0}</h3>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <div className="bg-white rounded-2xl border border-rakit-200 shadow-sm overflow-hidden">
                                <div className="p-6 border-b border-rakit-100 flex justify-between items-center">
                                    <h3 className="font-bold text-rakit-800 text-lg">Proyek Sedang Dikerjakan</h3>
                                </div>
                                <div className="divide-y divide-rakit-100">
                                    {activeProjects && activeProjects.length > 0 ? (
                                        activeProjects.map((project) => (
                                            <div key={project.id} className="p-6 hover:bg-rakit-50 transition flex flex-col md:flex-row justify-between gap-4">
                                        ...
                                        <div className="p-6 text-center text-gray-500">Belum ada proyek aktif.</div>
                                    )}
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="bg-white rounded-2xl border border-rakit-200 shadow-sm p-6">
                                ...
                                <div className="space-y-4">
                                    {newRequests && newRequests.length > 0 ? (
                                        newRequests.map((req) => (
                                            <div key={req.id} className="p-4 rounded-xl border border-rakit-100 bg-rakit-50/50 hover:border-rakit-300 transition cursor-pointer group">
                                                <h4 className="font-bold text-gray-800 group-hover:text-rakit-800 transition">{req.title}</h4>
                                                <div className="flex justify-between text-sm text-gray-500 mt-2">
                                                    <span>{req.location}</span>
                                                    <span className="font-bold text-rakit-600">{req.budget}</span>
                                                </div>
                                                {/* Tombol Ambil Penawaran bisa dibuat fungsional nanti */}
                                        ...
                                        <p className="text-sm text-gray-500 text-center">Belum ada permintaan baru.</p>
                                    )}
                                </div>
                            </div>
                        </div>
